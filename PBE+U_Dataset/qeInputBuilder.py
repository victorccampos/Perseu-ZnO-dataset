...
def transform_unit_cell(supercell_shape: np.ndarray,
    a: float,
    covera: float,
    template: str = 'ZnO_PBE+U_template.in'
) -> Atoms:
    """
    Transform a primitive cell to a supercell based on
    primitive cell scf file (template). 
    Template must have ibrav = 0 to be readable with ASE.
    """
    primitive_cell: Atoms = read(template, format='espresso-in')
    chemical_symbols: list[str] = primitive_cell.get_chemical_symbols()
    # ATOMIC_POSITIONS (crystal) of relaxed structure
    ...
    supercell.info['shape']: np.ndarray = supercell_shape
...
def get_qe_params(input_name: str) -> Tuple[dict, dict, tuple]:
    ...
    K_GRID = np.array([6, 6, 4]) # Primitive Cell K-GRID.
...
def make_pwscf_from_atoms(
    ...
    kpoints_supercell = K_GRID // supercell.info['shape']
    kpoints_supercell[kpoints_supercell == 0] = 1 # Ensure no zero k-points;
    
    
    write_espresso_in(
        file = filepath, atoms = supercell,
        input_data = NAMELIST, pseudopotentials = PSEUDOS, kpts = tuple(kpoints_supercell),
        koffset = (0,0,0),
...
if  __name__ == "__main__":
    ...
    celldm1_angstroms = 3.267457590699776
    celldm1_bohr = celldm1_angstroms / Bohr
    celldm3 = 1.5996
    ...
    # ======================= 1 FILE Creation ======================= #
    supercell_shape = np.array([3, 1, 2])
