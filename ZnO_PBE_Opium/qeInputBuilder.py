...
def transform_unit_cell(supercell_shape: Tuple[int, int, int],
    a: float,
    covera: float,
    template: str = 'ZnO_ibrav0_primcell.in'
) -> Atoms:
    """
...
if  __name__ == "__main__":
    # ======= Relaxed structure parameters for ZnO - Opium ======= #     
    celldm1_bohr = 6.100276
    ...
    atoms = transform_unit_cell(supercell_shape=supercell_shape, a=a, covera=covera)
    print(f"Creating supercell of size {supercell_shape}, with lattice constants:")
    print(f"{a = }")
    print(f"{covera = }")
