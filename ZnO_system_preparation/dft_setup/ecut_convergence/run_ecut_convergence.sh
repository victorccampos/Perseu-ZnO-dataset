#!/bin/bash

#### ecutwfc range to test
for ecutwfc_var in 30 50 70 90 110 130 150; do

ecutrho=$((4 * ecutwfc_var))

cat > ecut_$ecutwfc_var.in << EOF
&CONTROL
  calculation = 'scf',
  restart_mode = 'from_scratch',
  !restart_mode = 'restart',
  pseudo_dir = '../pseudos',
  outdir='.'
  prefix = 'ZnO_PBE_bulk',
  nstep=450,
  verbosity = 'high',
  tstress=.true.,
  tprnfor=.true.,
  forc_conv_thr=1.0d-5,
  etot_conv_thr=1.0d-6,
/
&SYSTEM
    ibrav= 4,
    celldm(1) = 6.194571412,
    celldm(3) = 1.614358356,
    ntyp= 2,
    nat=  4,
    ecutwfc =  $ecutwfc_var,
    ecutrho = $ecutrho,
    occupations = 'fixed',
    nbnd =  72,
    degauss = 0.005,
    smearing = 'gaussian',
    /
&ELECTRONS
    diagonalization = 'david'
    mixing_mode='local-TF',
    conv_thr= 1.0d-9,
    mixing_beta=0.1,
    electron_maxstep=190,
/

ATOMIC_SPECIES
Zn   63.380    Zn.upf
 O   15.999    O.upf

K_POINTS automatic
 12 12 12  0  0  0


ATOMIC_POSITIONS (crystal)
Zn            0.6666666667        0.3333333333        0.5006426381
Zn            0.3333333333        0.6666666667        0.0006426381
O             0.6666666667        0.3333333333        0.8796673619
O             0.3333333333        0.6666666667        0.3796673619
EOF

# Run no Quantum Espresso
mpirun -np 64 ../../bin/pw.x -i ecut_$ecutwfc_var.in > ecut_$ecutwfc_var.out
done
