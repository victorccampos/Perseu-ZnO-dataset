...
for k in 10 12 14 16 20 30 40; do
...
conda deactivate; nohup mpirun -np 64 ../../bin/pw.x -i kpoint.$k.in > kpoint.$k.out &

# Write the number of k-point and the total energy in calc-kpoint.dat
# regular expression : find for lines that startswith ! and
# get the 5th element (total energy in Ry) 

awk -v k="$k" '/^!/{printf "%d %s\n", k, $5}' kpoint.$k.out >> calc-kpoint.dat
