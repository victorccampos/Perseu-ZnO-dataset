...
function clean_run() {
    rm -rf anphon_NA0.{in,out} optimize.{in,out} tmp.lammps log.lammps *XFSET* xfset *.bands *.fcs *.xml *.ann
}
...
python plot_bands_parity.py zno222_NA0.bands.bak zno_lda_mlff.bands -o parity.png
