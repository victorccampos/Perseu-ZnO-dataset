...
def plot_convergence(df: pd.DataFrame, pseudopot_title: str, save: bool = False):
    ...
    fig, (axE, axdE) = plt.subplots(
        2, 1, sharex=True, figsize=(6, 5), dpi=300, gridspec_kw={"hspace": 0.05}
    )
    ...
    if save:
        formats = ["pdf", "png"]
        nome_figura = input(
            "Digite o nome da figura a ser salva:\nEx: minhafigura.{pdf,png}\n"
        )
...
if __name__ == "__main__":
    ...
    # Opium
    # scfs_path =  "/home/jvc/LDA_Study/pseudos/OPIUM/scf_outputs"
    # data = get_energies(scfs_path)
    # df = transform_data_to_df(data)
    # plot_convergence(df, pseudopot_title="OPIUM LDA NC", save = True)

    # PSLibrary
    # scfs_path =  "/home/jvc/LDA_Study/pseudos/pslibrary/scf_outputs"
    # data = get_energies(scfs_path)
    # df = transform_data_to_df(data)
    # plot_convergence(df, pseudopot_title="PSlibrary LDA USPP", save = True)

    # ONCV PBE
    scfs_path = "/home/jvc/ONCV_PBE/scf_outputs/"
    data = get_energies(scfs_path)
    df = transform_data_to_df(data)
    plot_convergence(df, pseudopot_title="ONVC PBE", save=True)
